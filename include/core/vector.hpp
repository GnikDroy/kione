...
namespace k2 
{
  ...
  struct Vector
  {
    ...
    {
      return std::accumulate(data.begin(), data.end(), static_cast<T>(0), [](auto left, auto right) {
    ...
    constexpr auto operator+(const Vector<Num, N> &other) const noexcept
    {
    ...
    template <arithmetic Num>
    constexpr auto operator+=(const Vector<Num, N> &other) noexcept
    {
    ...
    template <arithmetic Num>
    constexpr auto operator*(const Num &c) const noexcept
    {
    ...
    template <arithmetic Num>
    constexpr auto operator*=(const Num &c) noexcept
    {
    ...
    template <arithmetic Num>
    constexpr auto operator/(const Vector<Num, N> &other) const
    {
      Vector<decltype(data[0] / other[0]), N> new_vector;
    ...
    template <arithmetic Num>
    constexpr auto dot(const Vector<Num, N> &other) const noexcept
    {
  ...
  struct Vector<T, 2>
  {
    ...
    template <arithmetic Num>
    constexpr auto operator+(const Vector<Num, 2> &other) const noexcept
    {
      return Vector<decltype(x + other.x), 2>{x + other.x, y + other.y};
    }

    template <arithmetic Num>
    constexpr auto operator+=(const Vector<Num, 2> &other) noexcept
    {
      x += other.x;
      y += other.y;
      return *this;
    ...
    template <arithmetic Num>
    constexpr auto operator/(const Num &c) const
    {
      return Vector<decltype(x / c), 2>{x / c, y / c};
    }
    ...
    template <arithmetic Num>
    constexpr auto dot(const Vector<Num, 2> &other) const noexcept
    {
    ...
    template <arithmetic Num>
    constexpr bool operator==(const Vector<Num, 2> &other) const noexcept
    {
      return x == other.x && y == other.y;
    }
  ...
  struct Vector<T, 3>
  {
    ...
    template <arithmetic Num>
    constexpr auto operator+(const Vector<Num, 3> &other) const noexcept
    {
      return Vector<decltype(x + other.x), 3>{x + other.x, y + other.y,
                                              z + other.z};
    }

    template <arithmetic Num>
    constexpr auto operator+=(const Vector<Num, 3> &other) noexcept
    {
      x += other.x;
      y += other.y;
      z += other.z;
    ...
    template <arithmetic Num>
    constexpr auto dot(const Vector<Num, 3> &other) const noexcept
    {
    ...
    template <arithmetic Num>
    constexpr bool operator==(const Vector<Num, 3> &other) const noexcept
    {
      return x == other.x && y == other.y && z == other.z;
    }
...
struct fmt::formatter<k2::Vector<T, N>>
{
  constexpr auto parse(format_parse_context &ctx) { return ctx.begin(); }

  template <typename FormatContext>
  auto format(const k2::Vector<T, N> &vec, FormatContext &ctx)
  {
    if constexpr (N == 2)
    {
...
std::ostream &operator<<(std::ostream &out, const k2::Vector<T, N> &vec) { return out << fmt::format("{}", vec); }