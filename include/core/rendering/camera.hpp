...
namespace k2 {
    struct Camera {
        ...
        template<>
        struct ProjectionTraits<Projection::Perspective> {
            float fov;
            float aspect_ratio;
            float far_clip;
            float near_clip;
        ...
        template<>
        struct ProjectionTraits<Projection::Orthographic> {
            float left;
            float right;
            float top;
            float bottom;
        ...
        glm::mat4 get_projection() {
            return std::visit([](auto &&traits) {
                ...
                if constexpr (std::is_same_v<T, ProjectionTraits<Projection::Perspective>>) {
                    ...
                    return glm::ortho(traits.left, traits.right, traits.bottom, traits.top, traits.near_clip,
                                      traits.far_clip);
                } else {
