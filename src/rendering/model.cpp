...
namespace {
    ...
    std::uint64_t get_material_texture(const std::filesystem::path& material_path, ResourceManager& resources) {
    ...
    std::vector<Mesh::Material> process_materials(const std::filesystem::path& path,
    ...
    bool process_model(const std::filesystem::path& path, const tinyobj::ObjReader& reader,
        std::vector<Mesh>& meshes, ResourceManager& resources) {
        auto& attrib = reader.GetAttrib();
        auto& shapes = reader.GetShapes();

        auto materials = process_materials(path, reader.GetMaterials(), resources);
        ...
        for (auto& shape : shapes) {
            ...
            for (size_t face = 0; face < shape.mesh.num_face_vertices.size(); face++) {
                ...
                if (!material_indices.contains(material_id)) {
                    ...
                    material_groups.push_back(Mesh::MaterialGroup { .material = materials[material_id], .indices = {} });
                }
    ...
    bool load_model(const std::filesystem::path& path, std::vector<Mesh>& meshes, ResourceManager& resources) {
        ...
        return process_model(path, reader, meshes, resources);
    }

}

Model::Model(const std::filesystem::path& path, ResourceManager& resources)
    : path { path } {
    auto success = load_model(path, meshes, resources);
...
void Model::draw(const Program& program, ResourceManager& resources) {
