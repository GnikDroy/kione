...
void Renderer2D::draw(Scene& scene) {
    ...
    scene.registry.view<k2::TransformComponent, k2::SpriteComponent>().each(
        [&](auto entity, const auto&, const auto& sprite) {
            ...
            // Region is authored in pixels from the image's top-left. inset half texel.
            ...
            if (texture != nullptr && texture->width > 0 && texture->height > 0) {
                ...
                uv = { .x = (sprite.region.x + 0.5f) / tex_w,
                    .y = (tex_h - sprite.region.y - sprite.region.h + 0.5f) / tex_h,
                    .w = (sprite.region.w - 1.0f) / tex_w,
                    .h = (sprite.region.h - 1.0f) / tex_h };
            }
